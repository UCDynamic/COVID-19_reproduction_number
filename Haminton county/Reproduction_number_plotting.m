...
%     if true_ro.time_frame(i)==3 && i >=206
    % if i >= 196 && i <= 300
    if true_ro.time_frame(i)==1 && true_ro.SI_sd(i)==5
        x=x+1;
        t1(x,1)= true_ro(i,8);
        t1(x,2)= true_ro(i,10);
        t1(x,3)= true_ro(i,11);
...
plot(datic2,t1(1:45,1), 'LineWidth',3, 'Marker','o')
  ...
  legend('Model R_{e}-Training','Model R_{e}- Prediction','R_{e}- WT method','fontsize',18, 'FontWeight', 'bold')
  % xlim([11 60])
  ylim([0.6 1.5])
...
%     % if true_ro.time_frame(i)==3 && i >=206
% %     if i >= 788 && i <= 842
% if true_ro.time_frame(i) == 3 && true_ro.SI_sd(i)==5
%         x=x+1;
%         t1(x,1)= true_ro(i,8);
%         t1(x,2)= true_ro(i,10);
%         t1(x,3)= true_ro(i,11);
