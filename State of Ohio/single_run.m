...
[S_loop, I_loop, R_loop,L_loop, latency]=solve_loop_latency(env, param,T,1);
for i=1:88
    for j=1:T
S_loop_sum{i}(j)=0;
I_loop_sum{i}(j)=0; 
...
Pop_loop_OH(t) = S_loop_OH_True(t)+I_loop_OH_True(t)+R_loop_OH_True(t);
end

I_true=zeros(75,1);

for i = 2:45
    I_true(i) = I_true(i) + (I_loop_OH_True(i) - I_loop_OH_True(i-1));
end
...
ax.GridAlpha = 0.5;
subplot(3,1,3)
plot((1:30),R_loop_OH_static(1:30), 'LineWidth',3)
hold on
plot((30:T),R_loop_OH_static(30:T), 'LineWidth',3,'LineStyle','--')
hold on
...
ax.GridLineStyle = '--';
ax.GridAlpha = 0.5;
legend('Model Results-TRAINING','Model Results-TESTING','True Data','fontsize',18, 'FontWeight', 'bold')
sgt=sgtitle('State of Ohio: Timeframe-1');
sgt.FontWeight= 'bold';
sgt.FontSize = 25;
